  ...
  return (
    <main className="grid min-h-screen place-items-center bg-background px-6 text-center">
      <div className="max-w-sm">
        ...
        <div className="mx-auto mt-8 grid h-16 w-16 place-items-center rounded-2xl border border-destructive/40 bg-destructive/10 text-3xl text-destructive">
          ⛔
        </div>
        <h1 className="mt-5 text-xl font-bold">Not authorized</h1>
        <p className="mt-2 text-sm text-muted-foreground">
          This account doesn't have admin access. Ask an existing admin to grant your role, or sign
...
export function AuthForm() {
  ...
  return (
    <main className="relative grid min-h-screen place-items-center overflow-hidden bg-background px-6">
      ...
      <form
        onSubmit={submit}
        className="relative w-full max-w-sm space-y-4 rounded-2xl border border-border bg-card/80 p-7 backdrop-blur"
      >
        <div className="text-center">
          <div className="code-display text-2xl font-black tracking-[0.2em] text-primary">
            LAQTA
          </div>
          ...
          <p className="mt-3 text-xs text-muted-foreground">
            {mode === "signup" ? "First user becomes admin" : "Sign in to continue"}
          </p>
        </div>
        <input
          dir="ltr"
          type="email"
          placeholder={pick(T.email, "en")}
          value={email}
        ...
        <input
          dir="ltr"
          type="password"
          placeholder={pick(T.password, "en")}
          value={password}
        ...
        <button
          disabled={busy}
          className="w-full rounded-xl bg-primary px-6 py-3 font-bold text-primary-foreground transition hover:opacity-90 disabled:opacity-60"
        >
...
function AdminDashboard() {
  ...
  async function load() {
    ...
    const rows = (data || []).map((d) => ({
      ...d,
      status: d.status as EventRow["status"],
      config: { ...DEFAULT_CONFIG, ...(d.config as Partial<EventConfig>) },
    })) as EventRow[];
    setEvents(rows);
    setLoading(false);
      ...
      <header className="sticky top-0 z-40 border-b border-border bg-card/80 backdrop-blur">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-3">
          <div className="flex items-center gap-3">
            <div className="grid h-9 w-9 place-items-center rounded-xl bg-gradient-to-br from-primary to-[color:var(--warning)] font-black text-primary-foreground">
              L
            </div>
            <div>
              <div className="code-display text-sm font-black tracking-[0.18em] text-foreground">
                LAQTA
              </div>
          ...
          <button
            onClick={signOut}
            className="rounded-lg border border-border px-3 py-1.5 text-xs text-muted-foreground transition hover:bg-muted hover:text-foreground"
          >
            {pick(T.signOut, "en")}
          </button>
        </div>
          ...
          {!loading && filtered.length === 0 && (
            <div className="rounded-2xl border border-dashed border-border bg-card/50 p-10 text-center">
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                Create one to share a guest form, gallery and staff console.
              </p>
              <button
                onClick={() => setCreating(true)}
...
function EventRowView({
  ...
  ev: EventRow;
  count?: { guests: number; assets: number };
  onChange: () => void;
}) {
  ...
  const statusTone =
    ev.status === "live"
      ? "bg-[color:var(--success)]/20 text-[color:var(--success)] border-[color:var(--success)]/40"
      : ev.status === "dryrun"
    ...
    <div className="overflow-hidden rounded-2xl border border-border bg-card transition hover:border-primary/40">
      <div className="p-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <span
            className={`inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider ${statusTone}`}
          >
        ...
        <div className="mt-1 text-xs text-muted-foreground" dir="ltr">
          /{ev.slug} · PIN{" "}
          <span className="code-display text-foreground">{ev.staff_pin || "not set"}</span>
        </div>
        ...
        <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4">
          <button
            ...
            className="inline-flex items-center justify-center gap-1.5 rounded-lg border border-primary/40 bg-primary/10 px-3 py-2 text-xs font-semibold text-primary transition hover:bg-primary/20"
          >
            📷 Photos{count?.assets ? ` · ${count.assets}` : ""}
          </button>
          <button
            onClick={() => setShowRegs(true)}
            className="inline-flex items-center justify-center gap-1.5 rounded-lg border border-border px-3 py-2 text-xs font-semibold transition hover:bg-muted"
          >
            Registrations
          </button>
          <button
        ...
        <div className="mt-2 flex flex-wrap items-center justify-between gap-2">
          ...
          <button
            ...
            className="inline-flex items-center gap-1 rounded-lg border border-destructive/30 px-3 py-1.5 text-xs font-semibold text-destructive transition hover:bg-destructive/10"
            title="Delete event"
          >
            ✕ Delete
          </button>
      ...
      <div className="grid gap-px border-t border-border bg-border sm:grid-cols-3">
        ...
        <CopyLink
          label={ev.config.gallery?.mode === "public" ? "Public wall" : "Gallery (private)"}
          url={galleryUrl}
      ...
      {showQr && (
        <QrModal
          url={
            ev.config.registration === "none" || ev.config.gallery?.mode === "public"
              ? galleryUrl
              : formUrl
          }
          title={ev.name}
  ...
  return (
    <div className="rounded-lg border border-border bg-background px-3 py-2">
      ...
      <div
        className={
          small
            ? "mt-0.5 truncate text-sm font-semibold capitalize"
  ...
  return (
    <div className={`flex items-center gap-2 bg-card px-3 py-2 ${disabled ? "opacity-50" : ""}`}>
      <div className="min-w-0 flex-1">
        ...
        <div className="truncate text-xs text-foreground/80" dir="ltr">
          {url}
        </div>
      </div>
      <button
        onClick={copy}
        disabled={disabled}
        className="shrink-0 rounded-md border border-border px-2 py-1 text-[10px] font-bold uppercase tracking-wider transition hover:bg-muted disabled:cursor-not-allowed"
      >
        {copied ? "✓" : "copy"}
      </button>
      {!disabled && (
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="shrink-0 rounded-md border border-border px-2 py-1 text-[10px] font-bold uppercase tracking-wider transition hover:bg-muted"
        >
          ↗
        </a>
      )}
...
export function QrModal({
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/70 p-4" onClick={onClose}>
      <div
        className="grid w-full max-w-sm gap-3 rounded-2xl border border-border bg-card p-5 text-center"
      ...
      >
        ...
        <div
          className="break-all rounded-lg border border-border bg-background px-3 py-2 text-xs"
          dir="ltr"
        >
          {url}
        </div>
        <div className="grid grid-cols-3 gap-2 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">
          <button
            onClick={() => setSheet(12)}
            className="rounded-lg border border-border py-2 hover:bg-muted"
          >
            12 / page
          </button>
          <button
      ...
      {sheet !== null && (
        <QrSheet eventName={title} url={url} count={sheet} onClose={() => setSheet(null)} />
      )}
    </div>
  );
}
      ...
      >
        ...
        <div className="mt-4 flex justify-end gap-2">
          ...
          <button
            onClick={go}
            disabled={busy}
            className="rounded-lg bg-destructive px-4 py-1.5 text-sm font-bold text-destructive-foreground disabled:opacity-60"
          >
            {busy ? "…" : confirmLabel}
          </button>
        </div>
...
export function RegistrationsModal({ event, onClose }: { event: EventRow; onClose: () => void }) {
        ...
        .map((h) =>
          esc(
            h === "code"
              ...
              : h === "created_at"
                ? g.created_at
                : (g.form_data as Record<string, string>)[h],
          ),
        )
        .join(","),
    );
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/70 p-4" onClick={onClose}>
      <div
        className="grid w-full max-w-5xl gap-3 rounded-2xl border border-border bg-card p-5 max-h-[92vh] overflow-hidden"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="min-w-0">
            ...
            <div className="mt-1 flex flex-wrap items-center gap-3 text-xs">
              ...
              >
                <span
                  className={`h-1.5 w-1.5 rounded-full ${liveOn ? "bg-[color:var(--success)] animate-pulse" : "bg-muted-foreground"}`}
                />
                {liveOn ? "live" : "polling"}
              </span>
        ...
        <div className="grid gap-3 overflow-hidden md:grid-cols-[2fr_1fr]">
          ...
          <aside
            className="overflow-auto rounded-xl border border-border bg-background p-3"
...
function GuestDetail({ guest, onDelete }: { guest: GuestRow; onDelete: () => void }) {
  const [delivery, setDelivery] = useState<{ status: string; destination: string | null } | null>(
    null,
  );
  ...
  useEffect(() => {
    setNote(null);
    loadDelivery(); /* eslint-disable-next-line react-hooks/exhaustive-deps */
  }, [guest.id]);
  ...
  async function resend() {
    ...
    try {
      ...
      if (!accessToken) {
        setNote("Session expired — sign in again");
        setSending(false);
        return;
      }
      ...
      setNote(
        res.status === "sent"
          ? "Email sent"
          : res.status === "queued"
            ? "Queued (no email provider configured)"
            : res.status === "skipped"
              ? `Skipped: ${res.reason ?? "no email"}`
  ...
  return (
    <div className="space-y-3">
      <div className="flex flex-col items-center">
        ...
        <div className="code-display text-sm text-primary" dir="ltr">
          {guest.code}
        </div>
        <div className="mt-1 flex flex-wrap justify-center gap-1.5">
          <span
            className={`rounded-full px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider ${guest.consent ? "bg-[color:var(--success)]/20 text-[color:var(--success)]" : "bg-destructive/20 text-destructive"}`}
          >
            {guest.consent ? "consented" : "no consent"}
          </span>
          <span className="rounded-full bg-muted px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-muted-foreground">
            {guest.source}
          </span>
          <span
            className={`rounded-full px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider ${dTone}`}
          >
            {delivery ? `email ${delivery.status}` : "email —"}
          </span>
        </div>
        <button
          onClick={resend}
          disabled={sending}
          className="mt-3 rounded-lg border border-border px-3 py-1.5 text-xs font-semibold transition hover:bg-muted disabled:opacity-60"
        >
      ...
      <button
        onClick={onDelete}
        className="w-full rounded-lg border border-destructive/30 px-3 py-2 text-xs font-bold uppercase tracking-wider text-destructive transition hover:bg-destructive/10"
      >
        Delete registration
      </button>
    </div>
...
export function EventEditor({ event, onClose }: { event?: EventRow; onClose: () => void }) {
  ...
  async function save() {
    setErr(null);
    if (!pin || pin.length !== 6) {
      setErr("Staff PIN must be 6 digits");
      return;
    }
    setBusy(true);
    const payload = {
  ...
  async function clone() {
    ...
    await supabase.from("events").insert({
      name: event.name + " (dry run)",
      slug: event.slug + "-dryrun-" + Math.random().toString(36).slice(2, 5),
      status: "dryrun",
      config: event.config as unknown as never,
      staff_pin: genPin(),
    });
    setBusy(false);
    onClose();
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/70 p-4" onClick={onClose}>
      <div
        className="grid w-full max-w-5xl gap-0 overflow-hidden rounded-2xl border border-border bg-card lg:grid-cols-[2fr_1fr] max-h-[92vh]"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex flex-col overflow-hidden">
          ...
          <div className="flex items-center justify-between border-b border-border bg-card/95 px-5 py-3 backdrop-blur">
            ...
            <button onClick={onClose} className="rounded-lg border border-border px-2 py-1 text-xs">
              ✕
            </button>
          </div>
          {/* Tabs */}
          ...
          <div className="flex-1 space-y-4 overflow-auto p-5">
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                ...
                <Field label="Status">
                  <select
                    ...
                    className="input"
                  >
                    {["draft", "dryrun", "live", "archived"].map((s) => (
                ...
                <Field label="Staff PIN (6 digits)">
                  <div className="flex gap-2">
                    <input
                      ...
                      className="input flex-1 code-display"
                    />
                    <button
                      type="button"
                ...
                <Field label="Locale">
                  <select
                    value={config.locale}
                    onChange={(e) =>
                      setConfig({ ...config, locale: e.target.value as EventConfig["locale"] })
                    }
                    className="input"
                  >
                ...
                <Field label="Selfie at registration">
                  <select
                    ...
                    onChange={(e) =>
                      setConfig({ ...config, selfie: e.target.value as EventConfig["selfie"] })
                    }
                    className="input"
                  >
                ...
                <Field label="Registration">
                  <select
                    value={config.registration || "open"}
                    onChange={(e) =>
                      setConfig({ ...config, registration: e.target.value as "open" | "none" })
                    }
                    className="input"
                  >
                ...
                <Field label="Require approval before public">
                  <select
                    value={String(Boolean(config.gallery?.requireApproval))}
                    onChange={(e) =>
                      setConfig({
                        ...config,
                        gallery: { ...config.gallery, requireApproval: e.target.value === "true" },
                      })
                    }
                    className="input"
                  >
                ...
                {config.gallery?.mode === "public" && (
                  <div className="sm:col-span-2 rounded-lg border border-[color:var(--warning)]/50 bg-[color:var(--warning)]/10 px-3 py-2 text-xs text-[color:var(--warning)]">
                    <span className="font-arabic">
                      كل صور الفعالية رح تكون مرئية للجميع — للفعاليات العامة فقط
                    </span>
                    <br />
                    All event photos become visible to everyone — for open public events only.
                  </div>
                )}
              </div>
            )}
                ...
                <div className="grid gap-3 sm:grid-cols-3">
                  ...
                  <Field label="Text">
                    <input
                      ...
                      onChange={(e) =>
                        setConfig({ ...config, theme: { ...config.theme, text: e.target.value } })
                      }
                      className="h-10 w-full rounded"
                    />
                  </Field>
                  ...
                  <div className="space-y-2">
                    {config.theme.logoUrl && (
                      <div className="flex items-center gap-3 rounded-lg border border-border bg-card p-2">
                        ...
                        <button
                          type="button"
                          onClick={() =>
                            setConfig({ ...config, theme: { ...config.theme, logoUrl: "" } })
                          }
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <span className="text-[10px] text-muted-foreground">
                        PNG/JPG — resized & embedded automatically
                      </span>
                    </div>
                    <input
                      dir="ltr"
                    ...
                    <div key={i} className="rounded-lg border border-border bg-background p-2">
                      <div className="grid items-end gap-2 md:grid-cols-[100px_100px_1fr_1fr_70px_auto_auto_auto]">
                        ...
                        <select
                          value={f.type}
                          onChange={(e) =>
                            updateField(i, { type: e.target.value as EventField["type"] })
                          }
                          className="input"
                        >
                          {["text", "tel", "email", "number"].map((t) => (
                        ...
                        <input
                          value={f.label.ar}
                          onChange={(e) =>
                            updateField(i, { label: { ...f.label, ar: e.target.value } })
                          }
                          placeholder="عربي"
                        ...
                        <input
                          value={f.label.en}
                          onChange={(e) =>
                            updateField(i, { label: { ...f.label, en: e.target.value } })
                          }
                          placeholder="English"
                          className="input"
                          dir="ltr"
            ...
            {tab === "messages" && (
              <div className="grid gap-3 md:grid-cols-2">
                <Field label="Consent (AR)">
                  <input
                    value={config.consentText.ar}
                    dir="rtl"
                    onChange={(e) =>
                      setConfig({
                        ...config,
                        consentText: { ...config.consentText, ar: e.target.value },
                      })
                    }
                    className="input font-arabic"
                  />
                </Field>
                <Field label="Consent (EN)">
                  <input
                    value={config.consentText.en}
                    dir="ltr"
                    onChange={(e) =>
                      setConfig({
                        ...config,
                        consentText: { ...config.consentText, en: e.target.value },
                      })
                    }
                    className="input"
                  />
                </Field>
                <Field label="Success (AR)">
                  <input
                    value={config.successMessage.ar}
                    dir="rtl"
                    onChange={(e) =>
                      setConfig({
                        ...config,
                        successMessage: { ...config.successMessage, ar: e.target.value },
                      })
                    }
                    className="input font-arabic"
                  />
                </Field>
                <Field label="Success (EN)">
                  <input
                    value={config.successMessage.en}
                    dir="ltr"
                    onChange={(e) =>
                      setConfig({
                        ...config,
                        successMessage: { ...config.successMessage, en: e.target.value },
                      })
                    }
                    className="input"
                  />
            ...
            {tab === "advanced" && (
              <div className="grid gap-3 md:grid-cols-2">
                <Field label="Max photo MB">
                  <input
                    type="number"
                    value={config.limits.maxPhotoMB}
                    onChange={(e) =>
                      setConfig({
                        ...config,
                        limits: { ...config.limits, maxPhotoMB: +e.target.value },
                      })
                    }
                    className="input"
                  />
                </Field>
                <Field label="Max video MB">
                  <input
                    type="number"
                    value={config.limits.maxVideoMB}
                    onChange={(e) =>
                      setConfig({
                        ...config,
                        limits: { ...config.limits, maxVideoMB: +e.target.value },
                      })
                    }
                    className="input"
                  />
                ...
                <Field label="Allow download all">
                  <select
                    value={String(config.gallery.allowDownloadAll)}
                    onChange={(e) =>
                      setConfig({
                        ...config,
                        gallery: { ...config.gallery, allowDownloadAll: e.target.value === "true" },
                      })
                    }
                    className="input"
                  >
                ...
                <Field label="Show videos">
                  <select
                    value={String(config.gallery.showVideos)}
                    onChange={(e) =>
                      setConfig({
                        ...config,
                        gallery: { ...config.gallery, showVideos: e.target.value === "true" },
                      })
                    }
                    className="input"
                  >
          ...
          <div className="flex items-center justify-between gap-2 border-t border-border bg-card/95 px-5 py-3 backdrop-blur">
            {err ? (
              <p className="truncate text-sm text-destructive">{err}</p>
            ) : (
              <span className="text-xs text-muted-foreground">
                Changes save when you click {event ? "Save" : "Create"}
              </span>
            )}
            <div className="flex gap-2">
              {event && (
              ...
              <button
                onClick={save}
                disabled={busy || !name}
                className="rounded-xl bg-primary px-5 py-2 text-sm font-bold text-primary-foreground transition hover:opacity-90 disabled:opacity-60"
              >
                {busy ? "…" : event ? "Save" : "Create"}
              </button>
            </div>
          </div>
        </div>
    ...
    <div className="hidden border-l border-border bg-background p-5 lg:block">
      <div className="sticky top-5">
        ...
        <div className="mx-auto w-[260px] overflow-hidden rounded-[2rem] border-4 border-border bg-black p-3 shadow-2xl">
          ...
          >
            {config.theme.logoUrl ? (
              <img src={config.theme.logoUrl} alt="" className="mx-auto h-10 object-contain" />
            ) : (
              <div
            ...
            <button
              ...
              style={{ background: config.theme.primary, color: config.theme.background }}
            >
              تسجيل
            </button>
          </div>
...
export function PhotosModal({ event, onClose }: { event: EventRow; onClose: () => void }) {
  ...
  async function load() {
    ...
    (gs || []).forEach((g) =>
      guestMap.set(g.id, { code: g.code, name: (g.form_data as { name?: string })?.name || "" }),
    );
    ...
    const enriched = await Promise.all(
      display.map(async (r) => {
        ...
        const { data: webU } = await supabase.storage
          .from("media")
          .createSignedUrl(web.storage_path, 3600);
        let thumbSigned = webU;
        if (thumbChild && thumbChild.storage_path !== web.storage_path) {
          thumbSigned = (
            await supabase.storage.from("media").createSignedUrl(thumbChild.storage_path, 3600)
          ).data;
  ...
  const filtered = assets.filter((a) => {
    ...
    if (
      ...
      !(a.guestCode || "").toLowerCase().includes(q) &&
      !albumOf(a).toLowerCase().includes(q)
    )
      return false;
    return true;
  });
  ...
  const items: LightboxItem[] = filtered.map((a) => ({
    id: a.id,
  ...
  async function sweepOrphans() {
    ...
    const probe = await adminSweepEventOrphans({
      data: { eventId: event.id, accessToken, dryRun: true },
    }).catch(() => null);
    if (!probe) {
      window.alert("Sweep failed");
      return;
    }
    if (probe.orphans === 0) {
      window.alert(`No orphaned files (scanned ${probe.scanned}).`);
      return;
    }
    if (
      !window.confirm(
        `Delete ${probe.orphans} orphaned file(s) not linked to any photo or selfie? (scanned ${probe.scanned})`,
      )
    )
      return;
    const res = await adminSweepEventOrphans({ data: { eventId: event.id, accessToken } }).catch(
      () => null,
    );
  ...
  async function handleUpload(files: FileList | null) {
    ...
    for (const file of Array.from(files)) {
      try {
        ...
        const isImg =
          ...
          /\.(jpe?g|png|gif|webp|heic|heif|bmp|tiff?)$/.test(lowerName);
        const isVideo =
          file.type.startsWith("video/") || /\.(mp4|mov|m4v|webm|avi|mkv)$/.test(lowerName);
        ...
        const album =
          (relParts.length > 1 ? relParts[relParts.length - 2] : "") || uploadAlbum.trim();
        ...
        const { error: upErr } = await supabase.storage
          .from("media")
          .upload(`${basePath}.${ext}`, file, { contentType: file.type, upsert: false });
        ...
        if (!isVideo) {
          try {
            ...
            const { error: webUpErr } = await supabase.storage
              .from("media")
              .upload(webPath, web, { contentType: "image/jpeg", upsert: false });
            if (!webUpErr) {
            ...
            const { error: thUpErr } = await supabase.storage
              .from("media")
    ...
    setUploading(false);
    setUploadMsg(
      `${ok} uploaded${fail ? `, ${fail} failed` : ""}${event.config.gallery?.requireApproval ? " — pending your approval" : ""}`,
    );
    load();
  }
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/70 p-4" onClick={onClose}>
      <div
        className="grid w-full max-w-6xl gap-3 rounded-2xl border border-border bg-card p-5 max-h-[92vh] overflow-hidden"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="min-w-0">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <label
              className={`cursor-pointer rounded-lg bg-primary px-3 py-1.5 text-sm font-bold text-primary-foreground transition hover:opacity-90 ${uploading ? "opacity-60 pointer-events-none" : ""}`}
            >
            ...
            <label
              className={`hidden cursor-pointer rounded-lg border border-primary/40 bg-primary/10 px-3 py-1.5 text-sm font-bold text-primary transition hover:bg-primary/20 sm:inline-block ${uploading ? "opacity-60 pointer-events-none" : ""}`}
              title="Pick a folder; each sub-folder name becomes an album (desktop only)"
            >
              📁 Upload folder
              <input
                ref={folderRef}
            ...
            <button
              ...
              className="rounded-lg border border-border px-3 py-1.5 text-sm font-semibold transition hover:bg-muted disabled:opacity-50"
            >
              Download all
            </button>
            <button
              ...
              className="rounded-lg border border-border px-3 py-1.5 text-sm"
              title="Delete storage files not linked to any photo or selfie"
            >
              Sweep orphans
            </button>
            <button onClick={load} className="rounded-lg border border-border px-3 py-1.5 text-sm">
              Refresh
            </button>
            <button
              onClick={onClose}
        ...
        <div className="flex flex-wrap items-center gap-2">
          <div className="flex gap-1 rounded-lg border border-border bg-background p-0.5">
            {(["all", "linked", "orphan"] as const).map((f) => (
              <button
                key={f}
                onClick={() => setFilter(f)}
                className={`rounded-md px-3 py-1 text-[11px] font-bold uppercase tracking-wider transition ${filter === f ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-foreground"}`}
              >
                {f}
              </button>
            ))}
          ...
          <div className="flex gap-1 rounded-lg border border-border bg-background p-0.5">
            {(["all", "pending", "approved"] as const).map((f) => (
              <button
                ...
                className={`rounded-md px-3 py-1 text-[11px] font-bold uppercase tracking-wider transition ${approval === f ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-foreground"}`}
              >
                {f}
          ...
          {albums.length > 0 && (
            <div className="flex items-center gap-1">
              <select
                ...
                className="rounded-lg border border-border bg-background px-2 py-1.5 text-[11px] font-bold uppercase tracking-wider text-foreground outline-none"
              >
              ...
              {album !== "all" && (
                <button
                  onClick={() => {
                    const n = window.prompt("Rename album", album);
                    if (n && n.trim() && n.trim() !== album) renameAlbum(album, n.trim());
                  }}
          ...
          <input
            placeholder="Search album, guest name or code…"
            value={query}
            onChange={(e) => setQuery(e.target.value)}
            className="flex-1 min-w-[180px] rounded-xl border border-border bg-input px-3 py-1.5 text-sm outline-none focus:border-primary"
          />
        </div>
          ...
          {loading ? (
            ...
            <div className="grid place-items-center py-20 text-center">
              ...
              <p className="mt-3 text-sm text-muted-foreground">
                No photos yet. Click <span className="font-bold text-primary">+ Upload</span> or
                wait for staff to upload.
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-3 gap-2 sm:grid-cols-4 md:grid-cols-6">
              {filtered.map((a, i) => (
                ...
                >
                  ...
                  {a.kind === "video" && (
                    <span className="pointer-events-none absolute top-1 right-1 rounded bg-black/70 px-1 text-[9px] font-bold text-white">
                      VIDEO
                    </span>
                  )}
                  {a.approved === false && (
                    <span className="pointer-events-none absolute top-1 left-1 rounded bg-[color:var(--warning)]/90 px-1 text-[9px] font-bold text-black">
                      PENDING
                    </span>
                  )}
                  {(a.guestCode || albumOf(a)) && (
                    <span className="pointer-events-none absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/80 to-transparent px-1.5 py-1 text-start text-[10px] font-bold text-white">
                      {albumOf(a) && (
                        <span className="block truncate opacity-90">📁 {albumOf(a)}</span>
                      )}
                      {a.guestCode && (
                        <span className="code-display block truncate" dir="ltr">
                          {a.guestCode}
                        </span>
                      )}
                      {a.guestName && (
                        <span className="block truncate font-normal opacity-80">{a.guestName}</span>
                      )}
                    </span>
                  )}
                  <div className="absolute right-1 bottom-1 z-10 flex gap-1 opacity-100 transition sm:opacity-0 sm:group-hover:opacity-100">
                    {a.approved === false ? (
                      <button
                        ...
                        title="Approve"
                        className="rounded bg-[color:var(--success)] px-1.5 py-0.5 text-[10px] font-bold text-black"
                      >
                        ✓
                      </button>
                    ) : (
                      <button
                        ...
                        title="Unpublish"
                        className="rounded bg-[color:var(--warning)] px-1.5 py-0.5 text-[10px] font-bold text-black"
                      >
                        ↩
                      </button>
                    )}
