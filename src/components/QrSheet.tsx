...
export function QrSheet({ eventName, url, count, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-auto bg-black/80 print:bg-white" onClick={onClose}>
      <div
        className="qr-sheet-shell mx-auto my-6 max-w-[820px] rounded-2xl bg-white p-6 text-black shadow-2xl print:my-0 print:rounded-none print:p-0 print:shadow-none"
      ...
      >
        <div className="mb-4 flex items-center justify-between print:hidden">
          <div>
            ...
            <div className="text-sm text-neutral-700">
              Each card prints with a unique number — place one per table or seat.
            </div>
          </div>
          <div className="flex gap-2">
            ...
            <button
              onClick={doPrint}
              className="rounded-lg bg-black px-4 py-1.5 text-sm font-bold text-white"
            >
              Print / Save PDF
            </button>
          </div>
        ...
        <div className="grid grid-cols-3 gap-3 print:gap-2">
          {items.map((n) => (
            <div
              key={n}
              className="flex flex-col items-center justify-between rounded-xl border border-neutral-200 p-3 print:break-inside-avoid"
            >
              ...
              <div className="w-full text-center">
                ...
                <div className="mt-1 inline-block rounded-md bg-black px-2 py-0.5 text-[11px] font-black text-white">
                  #{n}
                </div>
              </div>
            </div>
        ...
        <div className="mt-3 text-center text-[10px] text-neutral-500 print:mt-1" dir="ltr">
          {url}
        </div>
      </div>
